...
const BitmapFont: Record<string, string[]> = {
...
export default class WelcomeMessagePlugin extends BasePlugin {
  ...
  private async BuildWelcomePng(Options: {
    ...
    const TitleFontSize = this.ResolveAdaptiveFontSize(Options.Title, 23, 19, 44, 92);
    ...
    const DescriptionY = 122 + TitleLines.length * TitleLineHeight + 10;
...
class PngCanvas {
  ...
  public DrawText(Text: string, X: number, Y: number, Scale: number, Color: PngColor): void {
    let CursorX = X;
