...
- `APP_HOST_PORT`: dashboard port exposed by Docker Compose. Defaults to `3000`; set another value for a second full Docker instance.
...
PostgreSQL and Redis are bound to `127.0.0.1` only and use automatic host ports, so multiple HyperBot projects can run at the same time without manually changing `5432` or `6379`. The application container reaches them through Docker's internal network. Redis requires `REDIS_PASSWORD`; PostgreSQL requires `POSTGRES_PASSWORD`; the provided scripts generate both.
